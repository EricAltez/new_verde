                      ...
                      return Column(
                        children: [
                          ...
                          Expanded(
                            ...
                            itemBuilder: (_, int idx) => Text("User Name: ${listOfDocs[idx].name}"),
                            )
                          ),
                          Expanded(
                            child: ListView.builder(
                              physics: const NeverScrollableScrollPhysics(),
                              itemCount: listOfDocs.length,
                              itemBuilder: (_, int idx) => Text("User Points: ${((listOfDocs[idx].organico) + (listOfDocs[idx].carton) + (listOfDocs[idx].plastico))}"),
                            )
                          ),
                          Expanded(
                            child: ListView.builder(
                              physics: const NeverScrollableScrollPhysics(),
                              itemCount: listOfDocs.length,
                              itemBuilder: (_, int idx) => Text("Scan Number: ${((listOfDocs[idx].organico) + (listOfDocs[idx].carton) + (listOfDocs[idx].plastico)).toString()}"),
                            )
                          ),
                           Expanded(
                            child: ListView.builder(
                              physics: const NeverScrollableScrollPhysics(),
                              itemCount: listOfDocs.length,
                              itemBuilder: (_, int idx) => Text("Organico: ${(listOfDocs[idx].organico).toString()}"),
                            )
                          ),
                           Expanded(
                            child: ListView.builder(
                              physics: const NeverScrollableScrollPhysics(),
                              itemCount: listOfDocs.length,
                              itemBuilder: (_, int idx) => Text("Carton: ${(listOfDocs[idx].carton).toString()}"),
                            )
                          ),
                           Expanded(
                            child: ListView.builder(
                              physics: const NeverScrollableScrollPhysics(),
                              itemCount: listOfDocs.length,
                              itemBuilder: (_, int idx) => Text("Plastico: ${(listOfDocs[idx].plastico).toString()}"),
                            )
                          ),
                        ]
                      );
